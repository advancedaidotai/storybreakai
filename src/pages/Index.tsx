...
const VIDEO_URL_EXTENSIONS = [".mp4", ".mov", ".webm", ".mkv", ".avi", ".m4v"];
...
type VideoSourceMode = "upload" | "url";
...
const Index = () => {
  ...
  // Video URL state
  const [videoSourceMode, setVideoSourceMode] = useState<VideoSourceMode>("upload");
  ...
  const handleUpload = useCallback(async () => {
    ...
    // URL mode: create project with URL, skip file upload
    if (videoSourceMode === "url") {
      ...
      try {
        ...
        if (fnError || !data?.project_id) {
          setError(data?.error || fnError?.message || "Failed to create project from URL.");
          setUploadState("idle");
          return;
        }
        setUploadState("done");
        toast({ title: "Project created!", description: "Heading to analysis now..." });
      ...
      <div className="flex gap-8">
        ...
        <div className="flex-[3] min-w-0 space-y-6">
          ...
          <div className={`${panelStyle} p-5`} style={{ backgroundColor: panelBg }}>
            ...
            {contentType === "feature_film" ? (
              ...
              <div className="space-y-4 animate-[fade-in_300ms_ease-out]" key="tv">
                <LabelledInput label="Show Title" required value={showTitle} onChange={(e) => setShowTitle(e.target.value)} placeholder="What's your show called?" />
                {touched && !showTitle.trim() && <p className="text-[11px] text-destructive -mt-2">We need a title to get started</p>}
                ...
                <LabelledInput label="Episode Title" value={episodeTitle} onChange={(e) => setEpisodeTitle(e.target.value)} placeholder="e.g. The Revelation" />
                <LabelledInput label="Studio / Network" value={tvStudio} onChange={(e) => setTvStudio(e.target.value)} placeholder="Which studio or network?" />
              </div>
            )}
          </div>
          ...
          <div className={`${panelStyle} p-5`} style={{ backgroundColor: panelBg }}>
            <div className="flex items-center gap-2.5 mb-4">
              <StepBadge n={3} />
              <h3 className="text-sm font-semibold text-foreground">Where will this air?</h3>
            </div>
            <div>
              <label className="block text-[10px] font-semibold uppercase tracking-[0.15em] text-muted-foreground/70 mb-1.5">
                Delivery Target
        ...
        <div className="flex-[2] min-w-0">
          <div className="sticky top-16 space-y-4">
            ...
            <div className={`${panelStyle} p-5`} style={{ backgroundColor: panelBg }}>
              <div className="flex items-center gap-2.5 mb-4">
                <StepBadge n={4} />
                <h3 className="text-sm font-semibold text-foreground">Add your video</h3>
              </div>

              {/* Source mode toggle */}
              <div className="flex gap-1 p-1 rounded-lg bg-surface-0/60 border border-border/15 mb-4">
                <button
                ...
                >
                  <CloudUpload className="h-3.5 w-3.5" /> Upload File
                </button>
                <button
                ...
                >
                  <Link2 className="h-3.5 w-3.5" /> Paste Video URL
                </button>
              </div>
              ...
              {videoSourceMode === "url" ? (
                ...
                <div className="space-y-3">
                  <div className="flex gap-2">
                    <Input
                      value={videoUrl}
                      onChange={(e) => { setVideoUrl(e.target.value); setUrlValid(false); setUrlError(null); }}
                      placeholder="Paste a link to your video (MP4, MOV, WebM...)"
                      className="bg-[hsl(220_25%_8%)] border border-border/20 rounded-lg px-3 h-10 text-sm text-foreground placeholder:text-muted-foreground/40 focus-visible:ring-1 focus-visible:ring-primary/50 flex-1"
                      disabled={isBusy}
                    />
                    <Button
                      size="sm"
                      variant="outline"
                      className="h-10 px-4 rounded-lg text-xs font-medium border-border/30 hover:border-primary/40 hover:bg-primary/5 shrink-0"
                      onClick={handleLoadVideoUrl}
                      disabled={isBusy || urlValidating || !videoUrl.trim()}
                    >
                      {urlValidating ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Load Video"}
                    </Button>
                  </div>
                  {urlError && (
                  ...
                  <p className="text-[10px] text-muted-foreground/50">
                    Works with direct video links, S3 URLs, and cloud storage links.
                  </p>
                </div>
              ) : (
                /* ── File Upload Mode ── */
                  ...
                  <div className="relative flex flex-col items-center justify-center gap-3 z-10 py-8 px-5">
                    {uploadState === "uploading" ? (
                      ...
                      <>
                        ...
                        <p className="font-medium text-sm text-primary">{uploadState === "validating" ? "Checking your video…" : "Setting things up…"}</p>
                      </>
                    ) : selectedFile ? (
                      ...
                      <>
                        ...
                        <div className="text-center">
                          ...
                          <p className="text-xs text-muted-foreground mt-1">
                            MP4, MOV, and ProRES masters up to {config.sizeLabel}
                          </p>
                        </div>
                        <button
            ...
            <div className="relative group/upload">
              ...
              >
                {isBusy ? (
                  <><Loader2 className="h-4 w-4 animate-spin mr-2" /> Working on it…</>
                ) : (
